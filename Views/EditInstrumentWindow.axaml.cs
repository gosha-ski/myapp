...
public partial class EditInstrumentWindow : Window
{
    ...
    {   
        ...
        InitializeComponent();
        InitializeComponentsSecond();
        InitializeDefaults();
    }

    private void InitializeComponentsSecond()
    {
        InstrumentModel instrument = DbHelper.GetInstrumentById(_instrumentId);
        if (instrument == null)
        {
            return;
        }
        TbModel.Text = instrument.Model;
        TbSerial.Text = instrument.SerialNumber;
        TbInventory.Text = instrument?.InventoryNumber?.ToString() ?? "";

        TbLowerLimit.Text = instrument?.LowerLimit?.ToString() ?? "";
        TbUpperLimit.Text = instrument?.UpperLimit?.ToString() ?? "";

        TbAccuracy.Text = instrument?.AccuracyClass?.ToString() ?? "";
        TbVariation.Text = instrument?.VariationLimit?.ToString() ?? "";
    ...
    {
        Console.WriteLine("EditInstrument");
        ...
        try
        {
            DbHelper.EditInstrument(instrument);
            OnSaved?.Invoke();
            this.Close();
        }
        catch (Exception ex)
