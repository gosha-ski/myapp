...
public partial class EditTemplateWindow : Window
{
    ...
    public EditTemplateWindow(int templateId)
    {
        InitializeComponent();
    ...
    {
        TemplateModel template = DbHelper.GetTemplateById(_templateId);
        if (template != null)
        {
            tbFullName.Text = template.FullName;
            tbSerialNumber.Text = template.SerialNumber;
            tbInaccuracy.Text = template.Inaccuracy.ToString();
            tbLowerLimit.Text = template.LowerLimit.ToString();
            tbUpperLimit.Text = template.UpperLimit.ToString();
            // Устанавливаем выбранный тип устройства
            //var deviceTypeItem = _cbDeviceType?.ItemsSource?.Cast<DeviceTypeModel>()
            //    .FirstOrDefault(dt => dt.Name == template.DeviceType);
            //if (deviceTypeItem != null)
            //{
            //    _cbDeviceType.SelectedItem = deviceTypeItem;
            //}
            //// Устанавливаем выбранный диапазон
            //if (_rangesByDeviceType.TryGetValue(template.DeviceType, out var ranges))
            //{
            //    _cbRange.ItemsSource = ranges;
            //    _cbRange.SelectedItem = template.CurrentRange;
            //}
            //// Устанавливаем выбранные единицы измерения
            //if (_unitsByDeviceType.TryGetValue(template.DeviceType, out var units))
            //{
            //    _cbUnit.ItemsSource = units;
            //    _cbUnit.SelectedItem = template.Units;
            //}
        }
    }
    ...
    private void BtnOkClicked(object? sender, RoutedEventArgs e)
    {
        ...
        DbHelper.EditTemplate(template);
